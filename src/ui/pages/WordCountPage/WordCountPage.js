...
class WordCountPage extends Component {
  ...
  parseFiles = async (event) => {
    ...
    if (suitableFiles.length === 0) {
      this.props.showMessage('Подходящих файлов не найдено! Разрешены только *.txt и *.zip', 'Ошибка')
    ...
    if (convertedFiles.length === 0) {
      this.props.showMessage('Подходящие файлы не найдены ! разрешены только *.txt ', 'Ошибка')
      this.setState({pageStatus: pageStatuses.FILES_SELECTION})
      return
    }
      ...
      suitableFiles.map(async (file) => {
        if (file.name.split('.').pop() === 'zip') {
          ...
          if (files.textFiles.length === 0) {
            this.props.showMessage(`Не найдено *.txt файлов в ${(file.name)}`, 'предупреждение')
          }
